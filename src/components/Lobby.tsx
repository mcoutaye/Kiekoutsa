                ...
                <div className="px-4 pb-4 pt-2 space-y-4" style={{ background: "var(--surface)" }}>
                  {s.gameMode === "cible" ? (
                    ...
                    <div className="space-y-3">
                      ...
                      {(s.playlistSwapsAllowed ?? 2) === 0 && (
                        <div className="flex items-center justify-between gap-3">
                          <span className="text-xs text-gray-300">Montrer les musiques piochées avant de jouer</span>
                          <Toggle
                            checked={s.showPlaylistTracks ?? true}
                            onChange={(v) => setSettings({ showPlaylistTracks: v })}
                          />
                        </div>
                      )}
                    </div>
